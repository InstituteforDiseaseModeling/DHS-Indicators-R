...
CREATE_FP_NEED <- function(IRdata){
  ...
  if (!is.null(IRdata)){
    ...
    # Add a check to ensure all required columns exist in the dataset before proceeding
    if (all(c("v502", "v312", "v362") %in% colnames(IRdata))) {
      IRdata <- IRdata %>%
        mutate(
      ...
      IRdata <- IRdata %>% mutate(fp_future_use = NA) # Set 'fp_future_use' to NA
      warning("One or more required columns ('v502', 'v312', 'v362') do not exist in the dataset. Skipping calculation for 'fp_future_use'.")
    }
  }
  IRdata
}